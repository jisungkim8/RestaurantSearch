...
import restaurant.dto.BoardDto;//DTO
import restaurant.board.util.FileUtil;
import restaurant.board.validator.BoardValidator;
...
public class BoardUpdateController {
	...
	public ModelAndView form(@RequestParam("seq") int seq){
		...
		BoardDto boardCommand =boardDao.selectBoard(seq);
		//boardModify.jsp로 이동
		return new ModelAndView("boardModifyView","command",boardCommand);
	}
	

	//수정을 다하고나서 수정버튼 눌렀다면(에러메세지(유효성검사)
	@RequestMapping(value="/boardUpdate.do",method=RequestMethod.POST)
	public String submit(@ModelAttribute("command") BoardDto command,BindingResult result){
		if(log.isDebugEnabled()){
			log.debug("BoardDto="+command);//toString()
		}
		//유효성 검사->(비밀번호)에러발생->에레메세지를 불러오게 설정
		new BoardValidator().validate(command, result);
		//만약에 에러가 발생이 되었다면
		if(result.hasErrors()){
			return "boardModifyView";//form()호출
		}
		...
		if(!board.getPwd().equals(command.getPwd())){
			...
			return "boardModifyView";//boardModify.jsp로 전환(새로 암호를 입력받기)
